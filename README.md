...
3. **动态词语分类**：按完整动作片段采集左右手套原始时间序列，统一重采样后用残差 TCN 学习动作随时间的变化，并以滑动窗口进行实时识别。

视觉模块也针对项目手套做了补检增强：保留黑色/银色手套的轮廓着色与掌背判断，并为白色高反光手套增加亮色低色度候选、局部边缘锐化、ROI 补检和双手结果融合。

## 目前可以做什么

- 通过摄像头实时检测最多两只手，每只手输出 21 个 MediaPipe Hands 关键点。
- 对深色或浅色手套执行额外的轮廓/边缘增强和局部 ROI 重试，并融合普通检测与补检结果。
...
- 采集约 2.56 秒的动态词语片段，保留左右手套全速原始流和可选视觉骨架流，并与静态样本分库存储。
- 使用 TensorFlow.js 训练五层膨胀残差 TCN，以 10 Hz 滑动窗口识别已训练的动态词语。
- 训练单手“触觉到骨架”回归模型，并仅凭一只手套驱动 Canvas 骨架动画。

## 当前局限

- 仓库没有预训练手语模型或可直接使用的训练数据。词汇表只是采集标签和动作提示，不代表这些词已经能被识别。
- 静态模型仍是按帧/快照工作的闭集分类器。新增 TCN 能建模固定窗口内的动作轨迹，但首版仍属于“孤立动态词语分类”：没有自动切分任意长手语流，也没有 CTC、语言模型、面部信息或手语语法，因此识别历史不等同于完整连续句子识别。
- 动态实时识别必须把 `__idle__`（静止/过渡）作为一个类别采集。没有该类时，Softmax 在无动作窗口中仍会被迫输出某个词语。
- 要得到可用的“仅手套推理”模型，仍需先采集同一动作下同步的视觉与手套数据。普通手语视频没有与手套信号一一对应，不能直接代替这部分配对数据。
- 手套视觉增强仍以通用 MediaPipe Hands 为基础，并不是专门训练的手套关键点模型。握拳、侧面、严重遮挡、双手交叉、过曝或手套与背景颜色接近时，关键点仍可能漏检、漂移或左右互换。
...
| `/collect-dynamic`   | 按完整动作片段采集动态词语的左右手套与视觉原始流   |
| `/train-dynamic`     | 把动态片段重采样后训练残差 TCN                     |
| `/translate-dynamic` | 使用滑动时序窗口实时识别孤立动态词语               |
...
视觉检测默认先处理原始摄像头画面。普通检测不足时，系统分别分析深色手套和白色低色度手套候选，避免把明暗区域直接粘成一个掩码；候选轮廓会被转成 MediaPipe 更容易识别的完整 RGB 手形，白色手套同时保留经过限制的局部褶皱与轮廓锐化，而不是改成纯二值边缘图。系统随后对每个候选区域进行 ROI 补检，并：

- 合并整帧检测和 ROI 检测，去除几何上重复的骨架；
- 使用跟踪 ID、位置和历史状态尽量保持左右手身份稳定；
- 在短时漏检时保留最近结果，减少骨架闪烁；
- 对原黑色/银色手套根据银色区域判断掌心、根据纯黑区域判断手背；白色手套两面颜色接近时保持方向未知，不凭亮度强行判断。

为了获得更稳定的结果，应使用均匀光线和与手套反差明显的低纹理背景：黑色手套使用浅色背景，白色手套优先使用深灰等哑光背景，并避免白墙、白色电脑和鼠标与轮廓重叠。双手先分开进入画面，再逐渐执行交叉动作；完整保留手掌、手指和一小段手腕；避免丝质表面被强光完全过曝。侧面、握拳和大面积重叠仍应作为重点采集场景，而不能只采集正对摄像头的展开手掌。
...
串口解析器默认根据包2数据段之后的下一帧头自动区分两种固件：144 字节包体对应 272 字节拼合帧，仅包含四元数；168 字节包体对应 296 字节拼合帧，额外包含加速度和姿态角。判长所需的下一帧头未收完整时会保留缓冲等待，不会按较长格式提前吞掉下一包。
...
动态采集与静态快照使用独立的 IndexedDB store，不会清空或混入原有 `samples/models`。每个片段分别保留左手套、右手套和视觉骨架的原始时间流；TCN 训练时再把手套流重采样为固定输入：

- 时间长度：`128` 帧；
- 目标采样率：`50 Hz`；
- 窗口时长：约 `2.56 s`；
- 单手每帧：`137` 个压力值 + `4` 个归一化四元数 + `1` 个存在标记，共 `142D`；
- 双手每帧：`142 x 2 = 284D`。

模型包含五个残差 TCN 块，膨胀率依次为 `1/2/4/8/16`，随后使用全局时间平均池化和 Softmax 分类。四元数在插值前会归一化并消除 `q/-q` 符号跳变；缺失的手用存在标记和零特征表示。动态训练模型保存到独立的 `dynamic_models`，不会被静态 `/translate` 自动加载。

`/translate-dynamic` 会缓存最近约 2.56 秒的全速手套帧，每 100 ms 推理一次。窗口未填满时不推理，结果还需通过置信度阈值、连续稳定确认和同词冷却后才写入历史。该历史只是已确认孤立词的顺序拼接，不执行句法纠错或连续句子解码。
...
3. 静态词汇在 `/collect` 采集快照；动态词汇在 `/collect-dynamic` 采集完整动作片段，并额外采集足量 `__idle__` 静止/过渡片段。每类应覆盖不同速度、角度、手指力度和自然重复。
4. 按采集轮次、日期或参与者保留独立测试集，避免只看相邻帧随机划分得到的准确率。
5. 静态链路使用 `/train` 和 `/translate`；动态链路使用 `/train-dynamic` 和 `/translate-dynamic`。调整置信度、稳定次数和冷却时间只能减少抖动，不能弥补训练集中缺失的动作变化。
6. 如需手套驱动骨架，在 `/train-skeleton` 训练后到 `/mocap` 验证单手输出。

## 后续方向

- 为实际使用的黑色/白色手套建立专用的分割、21 点关键点与遮挡标注数据集，训练专用模型替代启发式 ROI 补检。
- 在现有 TCN 孤立词分类之上增加自动动作边界检测、CTC/Transducer 解码和语言模型，支持真正的连续句子识别。
