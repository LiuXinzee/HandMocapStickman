...
Python 桥接脚本与浏览器直连解析器都支持两种固件，第二包长度逐包自动探测（看哪个长度之后正好接着下一个帧头），不需要手工配置：

- 固定帧头：`AA 55 03 99`
- 第一包：包序号 `0x01`，包含 128 字节传感器数据；整包共 134 字节
- 第二包：包序号 `0x02`，数据段 **144 或 168 字节**（整包 150 / 174 字节）
  - 144 = 128 字节传感器数据 + 16 字节 IMU 四元数 → 拼合 272 字节
  - 168 = 再加 12 字节加速度 + 12 字节姿态角 → 拼合 296 字节
- 拼合后：`[0:256]` 原始传感器值，`[256:272]` 四个小端 `float32` 四元数 `[w, x, y, z]`，
  296 字节帧另有 `[272:284]` 加速度、`[284:296]` 姿态角

> 第二包长度千万别写死。多吃或少吃 24 字节会啃穿下一个包 1 的帧头，导致包 1 全被跳过、
> 包 2 全都配不上对，**一帧都拼不出来**——表现就是"连上了但完全没数据"。

完整协议见 [protocol_notes.md](./protocol_notes.md)。
...
`acceleration` / `attitude` 仅在 296 字节帧（168 字节第二包固件）下有值，旧固件为 `null`。
