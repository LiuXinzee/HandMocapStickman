...
为什么要合并(照抄 labelMerge.ts 的结论,别再重新调研一遍):
  判据是**差异落在哪个通道上**,不是"都是代词"。
    你 = 食指指向对方、他 = 食指指向侧前方 → 两个都朝身体外指,只差一个绕重力轴的
    水平角 = 纯 yaw。特征里两条 IMU 通路都对 yaw 不变(相对首帧四元数消掉绝对朝向、
    重力投影天生 yaw 不变),所以这两个词在特征空间里是同一个点。手套是 ICM-42688
    六轴无磁力计,绝对 yaw 零点在硬件层就不存在。实测(yawDrift 探针,399 条):静止时
    yaw 漂移 P90 15.92°/s → 归零 10s 后累计 159°,而类间距只有 45°;归零点本身 P90
    也有 43.4°。软件绕不过去。
    我 = 食指指向**自己胸口** → 手收回身体、指尖接触胸部。接触在 60 路指压 +
    72 路掌压里是一个压力事件("指向空气"永远产生不出来),而且六轴丢的只有 yaw 一个
    自由度、重力把 pitch/roll 都钉死了。**所以「我」分得开,已经拆成独立类。**

  复数组里还留着「我们」:按同一条判据它也该拆,但复数三个词一条数据都没有,
  更没按接触变体重录过,现在拆等于凭空断言一个没验证过的可分性。这个不对称是
  有意的,别顺手改齐。

CTC 这条路同样要合并:句子里的「你」和「他」在触觉特征上仍然是同一个东西,
不合并的话 CTC 会在这两个类之间随机分配概率,连带把整句的对齐搅乱。

**这边只管"哪些标签塌成一个类"。** 塌完之后默认翻译成哪个成员(TS 侧的
defaultMember / selfMember)是顺句层的事,Python 只训练不翻译,故意不带那两个字段。
"""
...
MERGE_GROUPS: list[tuple[str, str, list[str], str]] = [
    # 「我」不在这里 —— 它指自己胸口、有接触,分得开,是独立类(见模块文档)
    ("merged_pron_sg", "你/他", ["you", "he"], _YAW_REASON),
