...
## 问题1: IndexedDB VersionError (已修复)
- 用户浏览器中已有 v4 数据库（之前骨架采集独立化版本创建的）
- 回滚后代码使用 DB_VERSION = 3
- `indexedDB.open(name, 3)` 打开已有的 v4 数据库会抛出 VersionError
- **修复**: 将 DB_VERSION 改为 4，并添加 skeleton_samples store 的创建逻辑

## 问题2: handResults 为 null
从截图分析：
...
最可能的原因: **MediaPipe CDN 加载问题** — 用户在中国大陆，cdn.jsdelivr.net 可能被墙或很慢
...
**最终结论**: 问题出在 setInterval 的 100ms 间隔内，handResults 可能在某些帧之间被设为 null（MediaPipe 检测不到手的帧），而 ref 更新不及时。但日志显示是持续为 null，不是间歇性的。
